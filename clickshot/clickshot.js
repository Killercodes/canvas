var Fireworks = function () {
	...
	self.init = function () {
		...
		self.canvas.width = self.cw = window.innerWidth;//window.width;;
		self.canvas.height = self.ch = window.innerHeight;///window.height;
	...
	Particle.prototype.draw = function () {
        ...
        self.ctx.fillStyle = "hsla(50, 100%, " + this.brightness + "%, " + this.alpha + ")";
        ;
        
        self.ctx.beginPath();
        self.ctx.arc(this.x, this.y, this.size + 15, 0, Math.PI * 2, true);
        self.ctx.closePath();
		...
		if (self.flickerDensity > 0) {
			...
			if (rand(0, inverseDensity) === inverseDensity) {
				...
				self.ctx.fillStyle =
					"hsla(" + this.hue + ", 100%, " + this.brightness + "%, " + randAlpha + ")";
				self.ctx.fill();
			}
		}
	};
	...
	self.bindEvents = function () {
		...
		window.addEventListener("mousedown", function (e) {
			...
			self.mx = e.pageX - 10;//self.canvasContainer.offset().left;
			self.my = e.pageY - 10;//self.canvasContainer.offset().top;
			self.currentHue = rand(self.hueMin, self.hueMax);
			self.createFireworks(self.cw / 2, self.ch, self.mx, self.my);

			window.addEventListener("mousemove.fireworks", function (e) {
				var randLaunch = rand(0, 5);
		...
		window.addEventListener("mouseup", function (e) {
			//window.removeEventListener("mousemove.fireworks");
		});
	};
